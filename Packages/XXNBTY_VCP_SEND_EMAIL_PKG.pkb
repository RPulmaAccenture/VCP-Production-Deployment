  ...
  */
  ...
  BEGIN
    ...
    IF p_ricefw_name = 'ALL_VCP_RICEFW' THEN
      v_subject := 'VCP Collection Report';
    ELSIF p_ricefw_name = 'ON_HAND_DATA' THEN
      v_subject := 'VCP On-Hand Collection Report';
    ELSIF p_ricefw_name = 'IN_TRANSIT' THEN
      v_subject := 'VCP In-Transit Collection Report';
    ELSIF p_ricefw_name = 'WORK_ORDERS' THEN
      v_subject := 'VCP Work Orders Collection Report';
    ELSIF p_ricefw_name = 'WIP' THEN
      v_subject := 'VCP WIP Component Demands Collection Report';
    ELSIF p_ricefw_name = 'SALES_ORDERS' THEN
      v_subject := 'VCP Sales Orders Collection Report';
    ELSIF p_ricefw_name = 'ITEM_COSTS' THEN
      v_subject := 'VCP Item Cost Collection Report';
    ELSIF p_ricefw_name = 'ALL_EBS_RICEFW' THEN
      v_subject := 'EBS Collection Report';
    ELSIF p_ricefw_name = 'BOM' THEN
      v_subject := 'EBS Bill of Materials Collection Report';
    ELSIF p_ricefw_name = 'FORMULA' THEN
      v_subject := 'EBS Formula/Recipe Collection Report';
    ELSIF p_ricefw_name = 'BATCH' THEN
      v_subject := 'EBS Batch Collection Report';
    ELSIF p_ricefw_name = 'CUSTOMER_INTERFACE' THEN
      v_subject := 'EBS Customer Collection Report';
    ELSE
       v_subject := 'Oracle Email for ' || p_ricefw_name;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    WHEN 'IN_TRANSIT' THEN
      ...
      v_rec.table_name       := 'msc_st_supplies';
      v_rec.table_type       := 'STAGING';
      v_rec.where_clause     := 'process_flag = 3 AND order_type in (11,12) AND TRUNC(creation_date) = TRUNC(SYSDATE)';
      v_rec.group_by_clause  := 'error_text';
      v_query(1)             := v_rec;
